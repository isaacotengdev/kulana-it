...
import { Plug, Hammer, ArrowLeft, Clock } from "lucide-react";
import type { Metadata } from "next";

export const metadata: Metadata = {
  title: "DBG Middleware Project | Kulana IT Solutions",
  description: "DBG Middleware Project — Kulana IT Solutions",
};

export default function DbgMiddlewarePage() {
  return (
    <>
      <Navbar />
        ...
        <section className="gradient-hero relative overflow-hidden py-28 text-white">
          ...
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <span className="inline-flex items-center gap-2 px-4 py-1.5 bg-white/10 border border-white/20 rounded-full text-sm font-medium text-cyan-300 mb-5">
              <Plug className="w-4 h-4" /> Middleware · DBG
            </span>
            <h1 className="text-5xl lg:text-6xl font-extrabold mb-6 leading-tight">
              DBG Middleware Project
            </h1>
          </div>
        </section>

        <section className="flex flex-col items-center justify-center py-32 px-4 text-center">
          <div className="w-24 h-24 rounded-3xl bg-cyan-50 border border-cyan-100 flex items-center justify-center mb-8">
            <Hammer className="w-12 h-12 text-cyan-600" strokeWidth={1.75} />
          </div>
          ...
          <p className="text-gray-500 text-lg max-w-md leading-relaxed mb-10">
            The DBG Middleware Project page is currently being built. Check back soon for full project details,
